...
int htc8x60_wifi_set_carddetect(int on);
int htc8x60_wifi_get_mac_addr(unsigned char *buf);
...
static void *htc8x60_wifi_mem_prealloc(int section, unsigned long size)
{
...
int __init htc8x60_init_wifi_mem(void)
{
	int i;
...
static struct resource htc8x60_wifi_resources[] = {
	[0] = {
...
static struct wifi_platform_data htc8x60_wifi_control = {
	.set_power	= htc8x60_wifi_power,
	.set_reset	= htc8x60_wifi_reset,
	.set_carddetect	= htc8x60_wifi_set_carddetect,
	.mem_prealloc	= htc8x60_wifi_mem_prealloc,
	.get_mac_addr	= htc8x60_wifi_get_mac_addr,
};

static struct platform_device htc8x60_wifi_device = {
	.name		= "bcm4329_wlan",
	.id		= 1,
	.num_resources	= ARRAY_SIZE(htc8x60_wifi_resources),
	.resource	= htc8x60_wifi_resources,
	.dev		= {
		.platform_data = &htc8x60_wifi_control,
	},
};

extern unsigned char *get_wifi_nvs_ram(void);

static unsigned htc8x60_wifi_update_nvs(char *str)
{
...
int htc8x60_wifi_get_mac_addr(unsigned char *buf)
{
	...
	printk("htc8x60_wifi_get_mac_addr = %02x %02x %02x %02x %02x %02x \n",
...
#else
	htc8x60_wifi_update_nvs("sd_oobonly=1\n");
#endif
	htc8x60_wifi_update_nvs("btc_params80=0\n");
	htc8x60_wifi_update_nvs("btc_params6=30\n");
	htc8x60_init_wifi_mem();
	ret = platform_device_register(&htc8x60_wifi_device);
	return ret;
}
