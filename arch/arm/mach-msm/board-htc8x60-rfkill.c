...
/* Control bluetooth power for htc8x60 platform */
...
/* bt on configuration */
static uint32_t htc8x60_bt_on_table[] = {
...
static uint32_t htc8x60_bt_off_table[] = {
...
static void htc8x60_config_bt_on(void)
{
	printk(KERN_INFO "[BT]-- R ON --\n");

	/* set bt on configuration*/
	config_bt_table(htc8x60_bt_on_table,
				ARRAY_SIZE(htc8x60_bt_on_table));
	mdelay(5);
...
static void htc8x60_config_bt_off(void)
{
	/* BT_RESET_N */
	gpio_set_value(HTC8X60_GPIO_BT_RESET_N, 0);
	mdelay(1);
	...
	config_bt_table(htc8x60_bt_off_table,
				ARRAY_SIZE(htc8x60_bt_off_table));
	mdelay(2);
...
{
	if (!blocked)
		htc8x60_config_bt_on();
	else
		htc8x60_config_bt_off();

	return 0;
}

static struct rfkill_ops htc8x60_rfkill_ops = {
	.set_block = bluetooth_set_power,
};

static int htc8x60_rfkill_probe(struct platform_device *pdev)
{
	int rc = 0;
...
static int htc8x60_rfkill_remove(struct platform_device *dev)
{
...
static struct platform_driver htc8x60_rfkill_driver = {
	.probe = htc8x60_rfkill_probe,
	.remove = htc8x60_rfkill_remove,
	.driver = {
		.name = "htc8x60_rfkill",
		.owner = THIS_MODULE,
	},
...
{
	return platform_driver_register(&htc8x60_rfkill_driver);
}

static void __exit htc8x60_rfkill_exit(void)
{
	platform_driver_unregister(&htc8x60_rfkill_driver);
}
