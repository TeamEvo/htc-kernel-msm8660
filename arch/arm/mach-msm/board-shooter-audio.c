...
void shooter_snddev_poweramp_on(int en)
{
	...
	if (en) {
		/* enable rx route */
		msleep(30);
		gpio_request(PM8058_GPIO_PM_TO_SYS(HTC8X60_AUD_SPK_ENO),
						"AUD_HP_EN");
		gpio_direction_output(PM8058_GPIO_PM_TO_SYS(HTC8X60_AUD_SPK_ENO), 1);
		...
		gpio_request(PM8058_GPIO_PM_TO_SYS(HTC8X60_AUD_SPK_ENO),
						"AUD_HP_EN");
		gpio_direction_output(PM8058_GPIO_PM_TO_SYS(HTC8X60_AUD_SPK_ENO), 0);
		if (!atomic_read(&aic3254_ctl))
			curr_rx_mode &= ~BIT_SPEAKER;
	}
}
