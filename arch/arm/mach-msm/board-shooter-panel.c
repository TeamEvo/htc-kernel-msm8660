...
static void shooter_3Dpanel_on(bool bLandscape)
{
	...
	if(system_rev >= 1) {
		...
		rc = pm8xxx_gpio_config(PM8058_GPIO_PM_TO_SYS(HTC8X60_3DLCM_PD), &pwm_gpio_config);
		if (rc < 0)
			pr_err("%s pmic gpio config gpio %d failed\n", __func__, HTC8X60_3DLCM_PD);
	}

	rc = pm8xxx_gpio_config(PM8058_GPIO_PM_TO_SYS(HTC8X60_3DCLK), &clk_gpio_config_on);
	if (rc < 0)
...
static void shooter_3Dpanel_off(void)
{
	...
	rc = pm8xxx_gpio_config(PM8058_GPIO_PM_TO_SYS(HTC8X60_3DCLK), &clk_gpio_config_off);
	if (rc < 0)
		pr_err("%s pmic gpio config gpio %d failed\n", __func__, HTC8X60_3DCLK);
