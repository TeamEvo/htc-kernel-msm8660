/* arch/arm/mach-msm/board-pyramid-audio.c
 *
...
void htc8x60_snddev_poweramp_on(int en)
{
	pr_aud_info("%s %d\n", __func__, en);
	if (en) {
		msleep(50);
...
void htc8x60_snddev_hsed_pamp_on(int en)
{
	pr_aud_info("%s %d\n", __func__, en);
	if (en) {
		msleep(50);
...
void htc8x60_snddev_hs_spk_pamp_on(int en)
{
	pr_aud_info("%s %d\n", __func__, en);
	if (en) {
		msleep(50);
...
void htc8x60_snddev_receiver_pamp_on(int en)
{
	pr_aud_info("%s %d\n", __func__, en);
	if (en) {
		if (!atomic_read(&aic3254_ctl))
...
void htc8x60_snddev_bt_sco_pamp_on(int en)
{
	/* to be implemented */
}
...
void htc8x60_snddev_imic_pamp_on(int en)
{
	...
	pr_aud_info("%s %d\n", __func__, en);
	htc8x60_snddev_bmic_pamp_on(en);
	if (en) {
...
void htc8x60_snddev_fmhs_pamp_on(int en)
{
	pr_aud_info("%s %d\n", __func__, en);
	if (en) {
...
void htc8x60_voltage_on(int en)
{
}

int htc8x60_get_rx_vol(uint8_t hw, int network, int level)
{
	int vol = 0;
...
void htc8x60_rx_amp_enable(int en)
{
	if (curr_rx_mode != 0) {
		...
		if (curr_rx_mode & BIT_SPEAKER)
			htc8x60_snddev_poweramp_on(en);
		if (curr_rx_mode & BIT_HEADSET)
			htc8x60_snddev_hsed_pamp_on(en);
		if (curr_rx_mode & BIT_RECEIVER)
			htc8x60_snddev_receiver_pamp_on(en);
		if (curr_rx_mode & BIT_FM_SPK)
			htc8x60_snddev_fmspk_pamp_on(en);
		if (curr_rx_mode & BIT_FM_HS)
			htc8x60_snddev_fmhs_pamp_on(en);
		atomic_set(&aic3254_ctl, 0);;
	}
}

int htc8x60_support_aic3254(void)
{
	return 1;
}

int htc8x60_support_back_mic(void)
{
	return 1;
}

void htc8x60_get_acoustic_tables(struct acoustic_tables *tb)
{
	switch (system_rev) {
	case 0:
	case 1:
...
int htc8x60_support_beats(void)
{
...
void htc8x60_enable_beats(int en)
{
	pr_aud_info("%s: %d\n", __func__, en);
	set_beats_on(en);
}

int htc8x60_is_msm_i2s_slave(void)
{
	/* 1 - CPU slave, 0 - CPU master */
	return 1;
}

void htc8x60_reset_3254(void)
{
...
void htc8x60_set_q6_effect_mode(int mode)
{
	pr_aud_info("%s: mode %d\n", __func__, mode);
	atomic_set(&q6_effect_mode, mode);
}

int htc8x60_get_q6_effect_mode(void)
{
...
static struct q6v2audio_analog_ops ops = {
	.speaker_enable	        = htc8x60_snddev_poweramp_on,
	.headset_enable	        = htc8x60_snddev_hsed_pamp_on,
	.handset_enable	        = htc8x60_snddev_receiver_pamp_on,
	.headset_speaker_enable	= htc8x60_snddev_hs_spk_pamp_on,
	.bt_sco_enable	        = htc8x60_snddev_bt_sco_pamp_on,
	.int_mic_enable         = htc8x60_snddev_imic_pamp_on,
	.back_mic_enable        = htc8x60_snddev_bmic_pamp_on,
	.ext_mic_enable         = htc8x60_snddev_emic_pamp_on,
	.stereo_mic_enable      = htc8x60_snddev_stereo_mic_pamp_on,
	.fm_headset_enable      = htc8x60_snddev_fmhs_pamp_on,
	.fm_speaker_enable      = htc8x60_snddev_fmspk_pamp_on,
	.voltage_on		= htc8x60_voltage_on,
};

static struct q6v2audio_icodec_ops iops = {
	.support_aic3254 = htc8x60_support_aic3254,
	.is_msm_i2s_slave = htc8x60_is_msm_i2s_slave,
};
...
static struct aic3254_ctl_ops cops = {
	.rx_amp_enable        = htc8x60_rx_amp_enable,
	.reset_3254           = htc8x60_reset_3254,
...
void htc8x60_aic3254_set_mode(int config, int mode)
{
...
static struct q6asm_ops qops = {
	.get_q6_effect = htc8x60_get_q6_effect_mode,
};
...
void __init htc8x60_audio_init(void)
{
	...
	/* PMIC GPIO Init (See board-htc8x60.c) */
