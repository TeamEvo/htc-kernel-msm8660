...
static struct platform_device *qs_s5k4e1_pdev;
...
static int qs_s5k4e1_vreg_enable(const struct msm_camera_sensor_info *sdata)
{
	int rc;

	pr_err("[CAM] %s camera vreg on\n", __func__);
...
static int qs_s5k4e1_vreg_disable(const struct msm_camera_sensor_info *data)
{
	int rc;
	...
	unsigned char *rxdata, int length)
{
	...
	if (i2c_transfer(qs_s5k4e1_client->adapter, msgs, 2) < 0) {
		pr_info("[CAM] qs_s5k4e1_i2c_rxdata faild 0x%x\n", saddr);
		return -EIO;
	}
	return 0;
}
				...
				unsigned char *txdata, int length)
{
	...
	if (i2c_transfer(qs_s5k4e1_client->adapter, msg, 1) < 0) {
		pr_info("[CAM] qs_s5k4e1_i2c_txdata faild 0x%x\n", saddr);
		return -EIO;
	}
	...
	unsigned short *rdata, int rlen)
{
	...
	if (rc < 0) {
		pr_info("[CAM] qs_s5k4e1_i2c_read 0x%x failed!\n", raddr);
		return rc;
	}
	*rdata = (rlen == 2 ? buf[0] << 8 | buf[1] : buf[0]);
//	pr_info("qs_s5k4e1_i2c_read 0x%x val = 0x%x!\n", raddr, *rdata);
	return rc;
}
	 ...
	 uint16_t wdata)
{
	...
	buf[3] = (wdata & 0x00FF);
//	pr_info("i2c_write_b addr = 0x%x, val = 0x%x\n", waddr, wdata);
...
static int32_t bridge_i2c_write_w(unsigned short waddr, uint16_t wdata)
{
	...
	buf[2] = (wdata & 0xFF00) >> 8;
	buf[3] = (wdata & 0x00FF);
//	pr_info("bridge_i2c_write_w addr = 0x%x, val = 0x%x\n", waddr, wdata);
	rc = qs_s5k4e1_i2c_txdata(BRIDGE_ADDR>>1, buf, 4);
	if (rc < 0) {
		pr_info("bridge_i2c_write_w failed, addr = 0x%x, val = 0x%x!\n",
			waddr, wdata);
	}
	return rc;
}
	...
	unsigned short *rdata, int rlen)
{
	...
	if (rc < 0) {
		pr_info("[CAM] bridge_i2c_read 0x%x failed!\n", raddr);
		return rc;
	}
	*rdata = (rlen == 2 ? buf[0] << 8 | buf[1] : buf[0]);
//	pr_info("bridge_i2c_read 0x%x val = 0x%x!\n", raddr, *rdata);
	return rc;
}
	...
	unsigned char *rdata, int rlen)
{
	...
	if (rc < 0) {
		pr_info("[CAM] qs_s5k4e1_eeprom_i2c_read 0x%x failed!\n", raddr);
		return rc;
	}
	for (i = 0; i < rlen; i++) {
		rdata[i] = buf[i];
//		pr_info("qs_s5k4e1_eeprom_i2c_read 0x%x index: %d val = 0x%x!\n",
//			raddr, i, buf[i]);
	}
	return rc;
}
...
err:
        printk("[CAM] ********* bridge reset failure ***** \n");
	bridge_i2c_write_w(0x53, 0x00);
	msleep(30);

bridge_fail:
        printk("[CAM] *********  bridge_fail ******* \n");
	return rc;
...
static void qs_s5k4e1_bridge_config(int mode, int rt)
{
	...
	pr_info("[CAM] qs_s5k4e1_bridge_config: mode: %d\n", mode);

	if (mode == MODE_3D) {
		pr_info("[CAM] qs_s5k4e1_bridge_config: mode: MODE_3D\n");
		...
		bridge_i2c_write_w(0x51, 0x1);
#ifdef FOUR_LANE_SETTING
		bridge_i2c_write_w(0x52, 0x1);
#else
		bridge_i2c_write_w(0x52, 0x0);
#endif
		bridge_i2c_write_w(0x06, 0x03);
//		bridge_i2c_write_w(0x14, 0x0C);//np
#ifndef FOUR_LANE_SETTING
		bridge_i2c_write_w(0x57, (0x0<<2 | 0x0));//np
#endif
		bridge_i2c_write_w(0x04, 0x2018);//np
...
#if 0 /*we stretch frame along line length pclk*/
...
static int32_t qs_s5k4e1_write_exp_gain(struct sensor_3d_exp_cfg exp_cfg)
{
	...
	if (gain > max_legal_gain) {
		pr_info("Max legal gain Line:%d\n", __LINE__);
		gain = max_legal_gain;
	}
//	pr_info("qs_s5k4e1_write_exp_gain : gain = %d line = %d\n", gain, line);
...
#else /*we stretch frame along frame length line*/
static int32_t qs_s5k4e1_set_fps(struct fps_cfg   *fps)
{
	uint16_t total_lines_per_frame;
	int32_t rc = 0;
...
static int32_t qs_s5k4e1_write_focus_value(uint16_t code_value)
{
	...
	if ((qs_s5k4e1_ctrl->cam_mode == MODE_2D_LEFT) ||
		...
		/* Left */
		bridge_i2c_write_w(0x06, 0x02);
		pr_info("[CAM] %s: Left Lens Position: %d\n", __func__,
			code_value);
		...
		if (af_i2c_write_b_sensor(code_val_msb, code_val_lsb) < 0) {
			pr_info("[CAM] move_focus failed at line %d ...\n", __LINE__);
			return -EBUSY;
		}
	}
	...
	if ((qs_s5k4e1_ctrl->cam_mode == MODE_2D_RIGHT) ||
		...
		pr_info("[CAM] %s: Right Lens Position: %d\n", __func__,
			code_value);
		code_val_msb = code_value >> 4;
		code_val_lsb = (code_value & 0x000F) << 4;
		code_val_lsb |= qs_s5k4e1_af_mode;
...
static int32_t qs_s5k4e1_write_left_focus_value(uint16_t code_value)
{
	uint8_t code_val_msb, code_val_lsb;
	/* Left */
	bridge_i2c_write_w(0x06, 0x02);
...
static int32_t qs_s5k4e1_write_right_focus_value(uint16_t code_value)
{
	uint8_t code_val_msb, code_val_lsb;
	/* Right */
	bridge_i2c_write_w(0x06, 0x01);
	...
	int32_t num_steps)
{
	...
	if (dest_step_position == qs_s5k4e1_ctrl->curr_step_pos) {
		pr_info("[CAM] %s cur and dest pos are same\n", __func__);
		pr_info("[CAM] %s cur_step_pos:%d\n", __func__,
	...
	if (qs_s5k4e1_ctrl->cam_mode == MODE_3D) {
		...
		if (qs_s5k4e1_write_left_focus_value(dest_lens_position) < 0) {
			CDBG("move_focus failed at line %d ...\n", __LINE__);
			return -EBUSY;
		}
		...
		if (qs_s5k4e1_write_right_focus_value(dest_lens_position) < 0) {
			CDBG("move_focus failed at line %d ...\n", __LINE__);
			return -EBUSY;
		}
...
static int32_t qs_s5k4e1_set_default_focus(uint8_t af_step)
{
	...
	if (qs_s5k4e1_ctrl->curr_step_pos) {
		...
		if (qs_s5k4e1_ctrl->cam_mode == MODE_3D) {
			rc = qs_s5k4e1_write_left_focus_value(
				qs_s5k4e1_step_position_table_left[0]);
			if (rc < 0)
				return rc;

			rc = qs_s5k4e1_write_right_focus_value(
...
static void qs_s5k4e1_3d_table_init(void)
{
	...
	/* Derive the Table for Left sensor */
	rc = qs_s5k4e1_eeprom_i2c_read_b(0x110, &af_data, 2);
	if (rc == 0) {
		CDBG("%s: Left Far data - %d\n", __func__, af_data);
		qs_s5k4e1_step_position_table_left[0] = af_data;
	} else {
		CDBG("%s: EEPROM data read error\n", __func__);
		return;
	}

	rc = qs_s5k4e1_eeprom_i2c_read_b(0x114, &af_data, 2);
	if (rc == 0) {
		CDBG("%s: Left Medium data - %d\n", __func__, af_data);
		qs_s5k4e1_step_position_table_left[2] = af_data;
	} else {
		CDBG("%s: EEPROM data read error\n", __func__);
		return;
	}
	...
	as per the Lens characteristics derive intermediate
	step based on COC1.5 */
	qs_s5k4e1_step_position_table_left[1] =
		qs_s5k4e1_step_position_table_left[0] +
		(qs_s5k4e1_step_position_table_left[2] -
		qs_s5k4e1_step_position_table_left[0])/2;
	CDBG("%s: Step between 150cm:100cm is %d\n", __func__,
		qs_s5k4e1_step_position_table_left[1]);

	rc = qs_s5k4e1_eeprom_i2c_read_b(0x118, &af_data, 2);
	if (rc == 0) {
		CDBG("%s: Left Short data - %d\n", __func__, af_data);
		qs_s5k4e1_step_position_table_left[6] = af_data;
	} else {
		CDBG("%s: EEPROM data read error\n", __func__);
		return;
	}
	...
	intermediate steps for the left sensor*/
	step = (qs_s5k4e1_step_position_table_left[6] -
		qs_s5k4e1_step_position_table_left[2])/4;

	/* Interpolate the intermediate steps between 100cm
	to 50cm based on COC1.5 */
	qs_s5k4e1_step_position_table_left[3] =
		qs_s5k4e1_step_position_table_left[2] + step;
	qs_s5k4e1_step_position_table_left[4] =
		qs_s5k4e1_step_position_table_left[3] + step;
	qs_s5k4e1_step_position_table_left[5] =
		qs_s5k4e1_step_position_table_left[4] + step;

	/* Extrapolate the steps within 50cm based on
	COC2 to converge faster. This range is beyond the 3D
	specification of 50cm */
	anchor_point_q2 = qs_s5k4e1_step_position_table_left[6] << 1;
	step_q2 = (qs_s5k4e1_step_position_table_left[6] -
		qs_s5k4e1_step_position_table_left[2]);

	for (i = 7; i < QS_S5K4E1_TOTAL_STEPS_3D; i++) {
		anchor_point_q2 += step_q2;
		qs_s5k4e1_step_position_table_left[i] = anchor_point_q2 >> 1;
	}

	/* Derive the Table for Right sensor */
	rc = qs_s5k4e1_eeprom_i2c_read_b(0x112, &af_data, 2);
	if (rc == 0) {
		CDBG("%s: Right Far data - %d\n", __func__, af_data);
		qs_s5k4e1_step_position_table_right[0] = af_data;
	} else {
		CDBG("%s: EEPROM data read error\n", __func__);
		return;
	}

	rc = qs_s5k4e1_eeprom_i2c_read_b(0x116, &af_data, 2);
	if (rc == 0) {
		CDBG("%s: Right Medium data - %d\n", __func__, af_data);
		qs_s5k4e1_step_position_table_right[2] = af_data;
	} else {
		CDBG("%s: EEPROM data read error\n", __func__);
		return;
	}
	...
	qs_s5k4e1_step_position_table_right[1] =
		qs_s5k4e1_step_position_table_right[0] +
		(qs_s5k4e1_step_position_table_right[2] -
		qs_s5k4e1_step_position_table_right[0])/2;
	CDBG("%s: Step between 150cm:100cm is %d\n", __func__,
		qs_s5k4e1_step_position_table_right[1]);

	rc = qs_s5k4e1_eeprom_i2c_read_b(0x11A, &af_data, 2);
	if (rc == 0) {
		CDBG("%s: Right Short data - %d\n", __func__, af_data);
		qs_s5k4e1_step_position_table_right[6] = af_data;
	} else {
		CDBG("%s: EEPROM data read error\n", __func__);
		return;
	}
	...
	/* Using the 100cm and 50cm calibration values
	as per the Lens characteristics derive
	intermediate steps for the Right sensor*/
	step = (qs_s5k4e1_step_position_table_right[6] -
		qs_s5k4e1_step_position_table_right[2])/4;

	/* Interpolate the intermediate steps between 100cm
	to 50cm based on COC1.5 */
	qs_s5k4e1_step_position_table_right[3] =
		qs_s5k4e1_step_position_table_right[2] + step;
	qs_s5k4e1_step_position_table_right[4] =
		qs_s5k4e1_step_position_table_right[3] + step;
	qs_s5k4e1_step_position_table_right[5] =
		qs_s5k4e1_step_position_table_right[4] + step;

	/* Extrapolate the steps within 50cm based on
	COC2 to converge faster. This range is beyond the 3D
	specification of 50cm */
	anchor_point_q2 = qs_s5k4e1_step_position_table_right[6] << 1;
	step_q2 = (qs_s5k4e1_step_position_table_right[6] -
		qs_s5k4e1_step_position_table_right[2]);

	for (i = 7; i < QS_S5K4E1_TOTAL_STEPS_3D; i++) {
		anchor_point_q2 += step_q2;
		qs_s5k4e1_step_position_table_right[i] = anchor_point_q2 >> 1;
	}
...
static int32_t qs_s5k4e1_sensor_setting(int update_type, int rt)
{
	...
	if (update_type == REG_INIT) {
		...
		if (CSI_CONFIG || qs_s5k4e1_ctrl->cam_mode == MODE_3D) {
		...
		if (!CSI_CONFIG) {
			...
			if (qs_s5k4e1_ctrl->cam_mode == MODE_3D) {//hack
#ifdef FOUR_LANE_SETTING
...
#else
			//qs_s5k4e1_csi_params.settle_cnt = 16;
#endif
			qs_s5k4e1_csi_params.hs_impedence = 0x0F;
			qs_s5k4e1_csi_params.mipi_driving_strength = 0;
...
{
	pr_info("[CAM] probe done\n");
	gpio_free(data->sensor_reset);
	qs_s5k4e1_vreg_disable(data);
	return 0;
}
	...
	qs_s5k4e1_probe_init_sensor(const struct msm_camera_sensor_info *data)
{
	...
	pr_info("[CAM] qs_s5k4e1_probe_init_sensor\n");
	if (!rc) {
	...
	pr_info("[CAM] qs_s5k4e1_probe_init_sensor is called\n");
	rc = qs_s5k4e1_i2c_read(0x0000, &chipid, 2);
	pr_info("[CAM] ID: %d\n", chipid);
	/* 4. Compare sensor ID to QS_S5K4E1 ID: */
	if (chipid != 0x4e10) {
		rc = -ENODEV;
		pr_info("[CAM] qs_s5k4e1_probe_init_sensor fail chip id mismatch\n");
		goto init_probe_fail;
	}
	goto init_probe_done;
init_probe_fail:
	pr_info("[CAM] qs_s5k4e1_probe_init_sensor fails\n");
	gpio_set_value_cansleep(data->sensor_reset, 0);
	qs_s5k4e1_probe_init_done(data);
init_probe_done:
	pr_info("[CAM] qs_s5k4e1_probe_init_sensor finishes\n");
	return rc;
}
/* camsensor_qs_s5k4e1_reset */

int qs_s5k4e1_sensor_open_init(struct msm_camera_sensor_info *data)
{
	int32_t rc = 0;
	pr_info("%s: %d\n", __func__, __LINE__);
	pr_info("[CAM] Calling qs_s5k4e1_sensor_open_init\n");

	qs_s5k4e1_ctrl = kzalloc(sizeof(struct qs_s5k4e1_ctrl_t), GFP_KERNEL);
	if (!qs_s5k4e1_ctrl) {
		pr_info("[CAM] qs_s5k4e1_init failed!\n");
		rc = -ENOMEM;
		goto init_done;
	}
	...
	if (rc < 0) {
		pr_info("[CAM] Calling qs_s5k4e1_sensor_open_init fail1\n");
		return rc;
	}
	pr_info("[CAM] %s: %d\n", __func__, __LINE__);

	qs_s5k4e1_vreg_enable(data);
	...
	rc = gpio_request(data->sensor_reset, "qs_s5k4e1");
	pr_info("[CAM] qs_s5k4e1_open_init_sensor\n");
	if (!rc) {
		pr_info("[CAM] sensor_reset = %d\n", rc);
		gpio_direction_output(data->sensor_reset, 0);
		mdelay(12);
...
/*Default mode is 3D*/
	...
	if (lens_eeprom_data[0] == 0xff &&
			lens_eeprom_data[1] == 0xff) {
		pr_info("[CAM] first two bytes of lsc eeprom are 0xff, "
			"use default lens shading table\n");
...
{
	int rc = 0;
	pr_info("[CAM] qs_s5k4e1_probe called!\n");
	...
	pr_info("[CAM] qs_s5k4e1_probe successed! rc = %d\n", rc);
	return 0;

probe_failure:
	pr_info("[CAM] qs_s5k4e1_probe failed! rc = %d\n", rc);
	return rc;
}
...
int qs_s5k4e1_2D_sensor_config(void __user *argp)
{
	...
	if (copy_from_user(&cdata,
		...
		return -EFAULT;
	mutex_lock(&qs_s5k4e1_mut);
	pr_info("[CAM] qs_s5k4e1_sensor_config: cfgtype = %d\n",
	cdata.cfgtype);
...
static int qs_s5k4e1_sensor_release(void)
{
	...
	qs_s5k4e1_vreg_disable(qs_s5k4e1_ctrl->sensordata);
	kfree(qs_s5k4e1_ctrl);
	qs_s5k4e1_ctrl = NULL;
	pr_info("[CAM] qs_s5k4e1_release completed\n");
	mutex_unlock(&qs_s5k4e1_mut);

	return rc;
}

/* QS S5K4E1 file init start */
/* Need to initialize the 3D settings so that it can get the correct numbers of sensor in HAL */

static const char *Qs_s5k4e1Vendor = "Lite-on";
static const char *Qs_s5k4e1NAME = "qs_s5k4e1";
static const char *Qs_s5k4e1Size = "5M";
...
{
	ssize_t ret = 0;
	sprintf(buf, "%s %s %s\n", Qs_s5k4e1Vendor, Qs_s5k4e1NAME, Qs_s5k4e1Size);
	ret = strlen(buf) + 1;
	return ret;
}
...
{
	ssize_t length;
	struct msm_camera_sensor_info *sdata = qs_s5k4e1_pdev->dev.platform_data;
	length = sprintf(buf, "%d\n", sdata->stereo_low_cap_limit);
	return length;
}

static struct kobject *android_qs_s5k4e1;
...
static int qs_s5k4e1_sysfs_init(void)
{
	int ret ;
	pr_info("[CAM] qs_s5k4e1:kobject creat and add\n");
	android_qs_s5k4e1 = kobject_create_and_add("android_camera", NULL);
	if (android_qs_s5k4e1 == NULL) {
		pr_info("[CAM] qs_s5k4e1_sysfs_init: subsystem_register " \
		"failed\n");
		ret = -ENOMEM;
		return ret ;
	}
	pr_info("[CAM] qs_s5k4e1:sysfs_create_file\n");
	ret = sysfs_create_file(android_qs_s5k4e1, &dev_attr_sensor.attr);
	if (ret) {
		pr_info("[CAM] qs_s5k4e1_sysfs_init: sysfs_create_file " \
		"failed\n");
		kobject_del(android_qs_s5k4e1);
	}

	ret = sysfs_create_file(android_qs_s5k4e1, &dev_attr_cam3Dmode.attr);
	if (ret) {
		pr_info("[CAM] qs_s5k4e1_sysfs_init: sysfs_create_file cam3Dmode failed\n");
		kobject_del(android_qs_s5k4e1);
	}

	ret = sysfs_create_file(android_qs_s5k4e1, &dev_attr_stereo_low_cap_limit.attr);
	if (ret) {
		pr_info("[CAM] qs_s5k4e1_sysfs_init: sysfs_create_file stereo_low_cap_limit failed\n");
		kobject_del(android_qs_s5k4e1);
	}

	return 0 ;
}
/* QS S5K4E1 File init end */

static int qs_s5k4e1_sensor_probe(struct msm_camera_sensor_info *info,
		struct msm_sensor_ctrl *s)
{
	...
	rc = qs_s5k4e1_sysfs_init();
	if (rc < 0)
		goto probe_fail;
	return rc;

probe_fail:
	pr_info("[CAM] qs_s5k4e1_sensor_probe: SENSOR PROBE FAILS!\n");
	return rc;
}
...
static int qs_s5k4e1_step_test(void *data, u64 *val)
{
	...
	rc = qs_s5k4e1_get_calibration_data
		(&cdata.data.sensor_3d_cali_data);
	if (rc < 0)
		pr_info("[CAM] %s: Calibration data read fail.\n", __func__);
...
static int __qs_s5k4e1_probe(struct platform_device *pdev)
{
	...
	pr_info("[CAM] qs_s5k4e1_probe\n");
	/*turn on power*/
	//sdata->pdata->camera_gpio_on();
	rc = qs_s5k4e1_vreg_enable(sdata);
	sdata->pdata->camera_gpio_on();
